...
f2="/tmp/disksinfo2"
#--------------------------------------------------------------------
df    | grep -v -E '( /dev| /sys| /proc| /var)' > $f1
df -i | grep -v -E '( /dev| /sys| /proc| /var)' > $f2
#--------------------------------------------------------------------
# Disks info, free space, free inodes
#--------------------------------------------------------------------
buffer=$(printf "<table border=\"1\">\n";
cat $f1 | awk -v FILE=$f2 -v LIM1=50 -v LIM2=80                     \
'{ getline line < FILE;
   split(line, s);
   split($0, k);
   { if (k[5] != "Use%") {
     k[5] = substr(k[5], 0, length(k[5])-1) + 0;
     { if (k[5] > LIM1)
       { if (k[5] > LIM2)
   ...
   { if (s[5] != "IUse%") {
     s[5] = substr(s[5], 0, length(s[5])-1) + 0;
     { if (s[5] > LIM1)
       { if (s[5] > LIM2)
